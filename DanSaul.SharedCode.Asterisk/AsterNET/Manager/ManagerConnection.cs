...
namespace AsterNET.Manager
{
	...
	public class ManagerConnection
	{
		...
		private void login(int timeout)
		{
			...
			if (reconnected)
			{
				Log.Error("Login during reconnect state.");
			...
			if (response is ChallengeResponse)
			{
				...
				{
					disconnect(true);
					Log.Error("Unable to create login key using MD5 Message Digest.", ex);
				...
				Log.Information("Determined Asterisk version: " + asteriskVersion);
		...
		protected internal bool connect()
		{
			...
			lock (lockSocket)
			{
				if (mrSocket == null)
				{
					Log.Debug("Connecting to {0}:{1}", hostname, port);
					try
					{
					...
					{
						Log.Debug("Connect - Exception  : {0}", ex.Message);
						result = false;
					}
					if (result)
					{
		...
		{
			Log.Warning("reconnect (init: {0}), reconnectCount:{1}", init, reconnectCount);
			if (init)
				reconnectCount = 0;
					...
					else
					{
						try
						{
							if (retryCount < reconnectRetryFast)
							{
								...
								Log.Debug("Reconnect delay : {0}, retry : {1}", reconnectIntervalFast, retryCount);
								Thread.Sleep(reconnectIntervalFast);
							}
							else
							{
								// slow down after unsuccessful attempts assuming a shutdown of the server
								Log.Debug("Reconnect delay : {0}, retry : {1}", reconnectIntervalMax, retryCount);
						...
						{
							Log.Debug("Reconnect delay exception : ", ex.Message);
						}
		...
		internal void SendToAsterisk(ManagerAction action, string internalActionId)
		{
			...
			Log.Debug("Sent action : '{0}' : {1}", internalActionId, action);
			if (sa == null)
		...
		public string BuildAction(ManagerAction action, string internalActionId)
		{
			...
			foreach (string name in getters.Keys)
			{
				...
				{
					Log.Error("Unable to retrieve property '" + name + "' of " + action.GetType(), ex);
		...
		{
			Log.Debug("Dispatch response packet : {0}", Helper.JoinVariables(buffer, ", ", ": "));
			DispatchResponse(buffer, null);
		}

		internal void DispatchResponse(ManagerResponse response)
		{
			Log.Debug("Dispatch response : {0}", response);
		...
		internal void DispatchResponse(Dictionary<string, string> buffer, ManagerResponse response)
		{
			...
			if (!string.IsNullOrEmpty(actionId))
			{
				...
				if (responseHandler != null)
				{
					...
					{
						Log.Error("Unexpected exception in responseHandler {0}\n{1}", response, ex);
			...
			Log.Debug("Reconnected - DispatchEvent : " + response);
		...
		{
			Log.Debug("Dispatching event: {0}", e);
			...
			if (e is ConnectEvent)
			{
				...
				Log.Debug("Connected via {0}", protocol);
				...
				else
				{
					...
					Log.Warning("Unsupported protocol version '{0}'. Use at your own risk.", protocol);
				}
				if (reconnected)
				{
					Log.Debug("Send Challenge action.");
					ChallengeAction challengeAction = new ChallengeAction();
					try
					{
					...
					{
						Log.Debug("Send Challenge fail : ", ex.Message);
						disconnect(true);
					}
					return;
				}
